...
function AdminDashboardContent() {
  ...
  // The cards compare the last 7 days with the 7 before, so 14 days is all that's needed. Rounded
  // and memoised so the argument object stays identical between renders (no resubscribe loop).
  const ordersWindow = useMemo(() => ({ from: Math.floor(Date.now() / 3_600_000) * 3_600_000 - 14 * DAY_MS, limit: 1000 }), []);
  const allOrders = useQuery(api.services.orders.getAllOrdersAdmin, ordersWindow);
