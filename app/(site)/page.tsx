...
export default function HomePage() {
  ...
  const heroImg = featured?.image || '/img/red.png';
  const heroName = featured?.name || 'Chili Super Red';
  const heroSku = featured?.sku ? '#' + featured.sku : '#SR—118';
  const heroGrade = featured?.grade || 'S';
        ...
        <div style={{ position: 'relative', maxWidth: 1280, margin: '0 auto', padding: '56px 28px 84px', display: 'grid', gridTemplateColumns: '1.03fr 0.97fr', gap: 48, alignItems: 'center', minHeight: 'calc(100vh - 70px)' }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20, maxWidth: 452 }}>
              {[['Grade', heroGrade], ['Tank', featured?.tankNumber || '—'], ['Price', featured ? fmtPeso(featured.price) : '—']].map(([k, v]) => (
                <div key={k}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ position: 'absolute', writingMode: 'vertical-rl', textOrientation: 'mixed', left: -6, top: '50%', transform: 'translateY(-50%)', fontFamily: mono, fontSize: 10, letterSpacing: '0.34em', textTransform: 'uppercase', color: 'oklch(0.60 0.02 40)' }}>{heroName} &middot; No. {heroSku}</div>
            <Link href={featured ? `/specimen-detail?id=${featured._id}` : '/catalog'} style={{ position: 'relative', width: 'min(72%, 460px)', aspectRatio: '1/1', display: 'block' }}>
              <div style={{ position: 'absolute', inset: 0, borderRadius: 999, background: 'radial-gradient(circle at 50% 44%, oklch(0.42 0.16 30), oklch(0.26 0.11 26) 52%, oklch(0.17 0.07 25) 100%)', boxShadow: '0 50px 90px -40px oklch(0.22 0.10 24 / 0.7), inset 0 0 0 1px oklch(0.70 0.12 80 / 0.35)', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, animation: 'dcSwim 7s ease-in-out infinite' }}>
                  <img src={heroImg} alt={heroName} style={{ position: 'absolute', left: '50%', top: '46%', transform: 'translate(-50%,-50%)', width: '96%', height: '78%', objectFit: 'contain', filter: 'drop-shadow(0 20px 34px oklch(0 0 0 / 0.5))' }} draggable={false} />
                </div>
              </div>
              ...
              <div style={{ position: 'absolute', top: '10%', left: '11%', fontFamily: mono, fontSize: 10, letterSpacing: '0.14em', color: 'oklch(0.90 0.02 60 / 0.75)' }}>{heroSku} &middot; CITES</div>
              <div style={{ position: 'absolute', top: '9%', right: '11%', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: 8, background: 'oklch(0.70 0.14 82 / 0.16)', border: '1px solid oklch(0.72 0.13 82 / 0.5)', fontFamily: serif, fontWeight: 700, fontSize: 15, color: 'oklch(0.80 0.13 84)' }}>{heroGrade}</div>
              <div style={{ position: 'absolute', bottom: '11%', left: 0, right: 0, textAlign: 'center' }}>
                <div style={{ fontFamily: serif, fontWeight: 700, fontSize: 22, color: 'oklch(0.97 0.01 82)', letterSpacing: '-0.01em' }}>{heroName}</div>
                <div style={{ fontFamily: mono, fontSize: 10, letterSpacing: '0.16em', textTransform: 'uppercase', color: 'oklch(0.88 0.02 60 / 0.7)', marginTop: 4 }}>{heroGrade !== '—' ? 'Grade ' + heroGrade + ' · ' : ''}{featured ? (featured.stock === 1 ? '1 of 1' : featured.stock + ' available') : '1 of 1'}</div>
              </div>
            </Link>
